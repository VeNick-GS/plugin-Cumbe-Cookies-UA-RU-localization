...
if (basename($_SERVER['PHP_SELF']) != 'index.php') { // backend only
	...
	i18n_merge('cbcookies', 'ru_RU');
	i18n_merge('cbcookies', 'ua_UA');
		...
		echo '<div style="float: left; width: 95%;">'."\n";
	        	echo '<p><label>'.i18n_r('cbcookies/message_lb').':</label>'."\n";
           		echo '<SELECT id="selmsg" NAME="sett[0]" style="border: 1px solid #AAAAAA; border-radius: 2px 2px 2px 2px; text-align: left; padding-right: 1px; width: 90%;" onclick="javascript:ncselect(&quot;txtmsg&quot;,this.value)">'."\n";
        	        	echo $optionmsg;
        	   	echo '</SELECT></p>'."\n";
	        	echo '<p><TEXTAREA id="txtmsg" style="border: 1px solid #AAAAAA; border-radius: 2px 2px 2px 2px; height: 40px; margin: 0 0 0 3px; width: 100%; text-align: left;" type="text" name="q_set">'.i18n_r("cbcookies/message$ndVal[0]").'</TEXTAREA></p>'."\n";
...
<?php
		...
		echo '<div id="'.$cbcookies_w.'" style="display:block;"><div class="container container-center"><div class="cbcookies-text alert text-center contrast" data-alert><a href="#" class="alert-close close"></a>'.i18n_r("cbcookies/message$cbmessage").'&nbsp;&nbsp;<a class="btn btn-primary" id="cbcookies_accept" href="javascript:acceptco(&quot;'.$cbcookies_w.'&quot;, &quot;cbcookies&quot;)" >'.$i18n['cbcookies/accept'].'</a>&nbsp;&nbsp;';
		if (trim($cbpage) != ''){
			$P_lnk = find_url($cbpage,'');
			echo '<a class="btn btn-primary" id="cbcookies_more" href="http://wikipedia.org/wiki/HTTP_cookie">'.$i18n['cbcookies/more_in'].'</a>';
		}
		echo '</div></div></div>';
